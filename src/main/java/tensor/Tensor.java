...
public class Tensor implements ITensor {
    ...
    @Override
    public Tensor broadcast(Tensor other) {
        return new Tensor(data.broadcast(other.data));
    }

    @Override
    public Tensor broadcast(int[] shape) {
        return new Tensor(data.broadcast(Arrays.stream(shape).asLongStream().toArray()));
    }
